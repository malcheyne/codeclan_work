...
1. Be familiar with the different types of table relationships
2. Understand how keys create relationships
3. Be able to use all join types in R
4. Be able to identify the correct join to use
...
So far we have been extracting, manipulating and tidying data from **single tables**. But more often that not, you'll be working with data stored across multiple excel sheets, and you'll want them all to be in one table. This is known as "joining".   


Three questions arise when working with multiple datasets:<br>
...
<center>**"If so, how do we do this efficiently in R ?"**</center><br>

This lesson will cover the theory behind this, and show you how to do some simple joins in R. 
...
where 'one' and 'many' refer to records (i.e. rows) in each of the tables. So the phrase 'one-to-one' should be read in full as 'one row in table A is linked to one row in table B', and so on. We'll discuss these relationships one at a time with examples of their use, but first we need to talk about **keys**!


## Keys


Relationships between tables are set up by adding keys. There are two different types. 

**Primary key** columns are those which establish a unique identifier for a table. 


**Foreign key columns** are columns in a table that contains values corresponding to the primary keys of **another** table. 


Let's look at an example. Imagine you have two datasets : `animals` and `diets`, which correspond to the diets different animals at the zoo are on.  
...
A primary key is a key which uniquely identifies each observation in a table. So, in our `animals` dataset, we have a primary key called `id`. From this particular column, you could identify each animal in it based on their id number. 

In our `diets` table, the primary key is also `id`, because this uniquely identifies the particular diet each animal is on.  

Now, for the **foreign keys**. In this case, we have one foreign key, which is in the `animals`. The `diet_id` column of table `animals` is a `FOREIGN KEY`: it contains values corresponding to the `PRIMARY KEY`s of the `diets` table. If you look at the animals dataset, you'd be able to find out which diet they're on from comparing the `id` number to the one in the `diets` dataset. 
...
We've looked at the three types of relationships that can exist between tables. Now it's time to answer the remaining two questions:
...
The `dplyr` package contains functions to perform all of the joins we might need. 
...
Let's start with an example, here's a table of customers: 
...
**Task - 5 mins** 
Think back to our definitions of keys:
...
```
This would take entries where there is a `customer_ID`, and matching `payment_type.`
...
### Adding columns - `bind_cols()` in `dplyr`

To merge two datasets horizontally (i.e. add on the columns to the side, rather than add rows to the bottom), you can use the `bind_cols()` function. Note that this function doesn't take in a column to match records on, so for this binding to be meaningful, the order of records in your first table has to match the order in your second table.
...
### Adding rows - `bind_rows()` in `dplyr`

To join two datasets vertically (i.e. add rows to the  bottom, not columns to the side), use the `bind_rows()` function. For this binding to make sense, the two tables must contain the same variables.
