...
```{r}
thought_police <- "[tT]hought [pP]olice"
whole_text %>% 
  str_detect(thought_police)

whole_text %>%
  str_count(thought_police)  
...
```{r}
big_brother <- "[bB]ig [bB]rother"
big_brother <- "(?i) big( +)brother"
whole_text %>%
  str_count(big_brother) 

# 2 with lower case, 10 with upper case
```
...
# No he doesn't mention her name
```
...
```{r}
date <- "[A-z][a-z]+ [0-9]+[a-z]+[:punct:] [0-9]{4}"
whole_text %>%
  str_extract_all(date)

# April 4th, 1984 is mentioned twice
```
...
winston <- "[wW]inston"
whole_text %>% 
  str_replace_all(winston, "--------")

# Has replaced winston with --------
```
...
```{r}
gin <- " [gG]in[:punct:] | GIN[:punct:] "
whole_text %>% 
  str_replace_all(gin, "[vV]odka|VODKA")
...
```{r}
ministries <- "[mM]inistries"
departments <- "[dD]epartments"
